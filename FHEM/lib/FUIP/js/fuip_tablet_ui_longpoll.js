...
var ftui = {
	...
	setConnected: function() {
		if(ftui.poll.status != 1)
			return;
		ftui.showErrorOverlay(false);
		if(ftui.poll.long.openTimer) {
			clearTimeout(ftui.poll.long.openTimer);
			ftui.poll.long.openTimer = null;
		};	
    ...
    createLongpoll: function () {
		...
		ftui.poll.long.request = $.ajax({
			...
			data: {
				...
				inform: 'type=status;filter=' + ftui.poll.long.filter + ';since=' +
						ftui.poll.long.lastEventTimestamp + ';fmt=JSON',
			...
			xhr: function () {
					...
					ftui.poll.long.xhr.addEventListener('readystatechange', function (e) {
						if(e.target.readyState === 1) {  // opened
							ftui.poll.long.openTimer = setTimeout(ftui.setConnected,1500);
						};
						...
						if (e.target.readyState === 3) { // loading, i.e. data received
							ftui.setConnected();
							ftui.handleUpdates(e.target.responseText);
						}
        ...
        .fail(function (jqXHR, textStatus, errorThrown) {	
			if(ftui.poll.status != 0 &&  ftui.poll.status != 3) // not if disconnected or disconnecting
				ftui.showErrorOverlay(true);
			var reason = textStatus + ": " + errorThrown;
			ftui.log(1, "Longpoll error " + reason + ", restarting...","base.poll");	
        })
		.always(function(){ftui.conditionalConnect(false)});
    },
	...
	connect: function() {
		...
		//			success: handle results
			ftui.shortPoll().then(ftui.createLongpoll, ftui.conditionalConnect);
		}else{
			ftui.createLongpoll();
		};	
	},	
	...
	// PFE
	disconnect: function() {
		ftui.poll.status = 3;  // disconnecting
		if (ftui.poll.long.request)
